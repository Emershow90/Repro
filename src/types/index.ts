...
export interface Log {
  ...
  enderecos?: number;
  eph?: string | number;
  mediaPorEndereco?: string | number;
}
...
export type ReplenishmentStep = 'ENDERECO' | 'CONTENANT' | 'LOTE' | 'ARTIGO' | 'QUANTIDADE' | 'CONFIRMACAO';
...
// -------------------------------------------------------------
// GENEALOGIA E RASTREABILIDADE DE ESTOQUE (CTN PAI / ARTIGO / LOTE)
// -------------------------------------------------------------

export interface CtnChildBoxGroup {
  id: number | string;
  caixaInicio: string;
  caixaFim: string;
...
export interface CtnArticleNode {
  artigo: string;
  totalCaixas: number;
  totalPecasEstimadas: number;
  gruposFilhos: CtnChildBoxGroup[];
  ruasDestino: string[];
  operadores: string[];
  divergenciasDetectadas: number;
  possuiRegraCadastrada: boolean;
  quantidadePadraoRegra?: number;
}

export interface CtnGenealogyReport {
  termoPesquisado: string;
  ctnPai: string;
  totalCaixas: number;
  totalArtigosDistintos: number;
  totalEventos: number;
  artigos: CtnArticleNode[];
  ruasAtendidas: string[];
  operadoresEnvolvidos: string[];
  primeiraMovimentacaoTs?: number;
  ultimaMovimentacaoTs?: number;
  status: 'COMPLETO' | 'PARCIAL' | 'DIVERGENTE';
  auditoriasRelacionadas: AuditLog[];
  logsBrutos: Log[];
}
